...
function ADashboard() {
  ...
  useEffect(()=>{
    authFetch('/api/get_testData',{
      'methods':'GET',
    })
    .then(r => r.json())
    .then((r) => {
      // console.log(r)
      if(r.testData !== undefined )
      settestData(r.testData);
    })
      ...
      <Holder>
        ...
        <div style = {{float:"left",width:"400px",height:"60vh",backgroundColor:"white", padding:"1.8rem", marginRight:"10px"}}>
          <h3>Tests log:</h3>
            ...
            {testData.map((item,i)=>(
              <span key={i}>
              <p><b>Test Id</b>: ({item.test_id})</p><br/>
              <p><b>Attempted</b>: ({item.attempted})</p><br/>
              <p><b>Total Registered</b>: ({item.total_registered})</p><br/>
              {/* //change time */}
              </span>
            ))}
            </div>
